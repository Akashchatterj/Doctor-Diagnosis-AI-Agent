...
Entrez.email = "chatterjeeakash887@gmail.com"
...
def get_openai_client(api_key):
    """
    Create OpenAI client that automatically detects the API key type.
    Supports both Euri API keys (euri-xxx) and standard OpenAI keys (sk-xxx)
    """
    if not api_key:
        return None
    
    # Auto-detect Euri API key
    if api_key.startswith('euri-'):
...
def extract_findings_and_keywords(analysis_text):
    ...
    common_terms = [
    # infections / lung
    "pneumonia", "infiltrates", "opacities", "ground glass", "ground-glass",
    "consolidation", "cavitation", "bronchiectasis", "atelectasis",
    "air bronchogram", "hyperinflation", "pneumonitis",

    # masses / nodules
    "nodule", "mass", "lesion", "tumor", "metastasis", "malignancy",
    "neoplasm", "adenopathy", "lymphadenopathy",

    # pleura / heart
    "effusion", "hydropneumothorax", "pleural thickening", "pneumothorax",
    "empyema", "cardiomegaly", "mediastinal widening", "pericardial effusion",

    # vascular
    "embolism", "pulmonary embolism", "aneurysm", "dissection",

    # edema / CHF
    "edema", "interstitial edema", "vascular congestion",

    # chronic lung disease
    "fibrosis", "emphysema", "honeycombing", "interstitial lung disease",
    "reticulation", "bullae",

    # trauma
    "fracture", "rib fracture", "spine fracture", "contusion",

    # others
    "calcification", "silhouette sign", "airspace disease", "patchy opacities",
    "volume loss"
]
...
def analyze_image(image, api_key, enable_xai=True):
    ...
    # Use the helper function to get the right client
    client = get_openai_client(api_key)
    if not client:
        return {
            "id": str(uuid.uuid4()),
            "analysis": "Invalid API key provided.",
            "findings": [],
            "keywords": [],
