...
import time
import openai
from utils_simple import get_openai_client

# Chat system storage checking any existing chat or not
def get_chat_store():
...
def get_messages(case_id,limit=50):
    """get the recent message from a chat room"""
    store = get_chat_store()
    
    if case_id in store["room"]:
...
def get_openai_response(user_question, case_description, findings=None, api_key=None):
    ...
    # Use the utility function to get the right client
