...
def render_qa_chat_interface():
    ...
    # CRITICAL FIX: Message input MUST be at the root level, not inside tabs/expanders/columns
    # Only show if we have an active room
    if "current_qa_id" in st.session_state:
        qa_id = st.session_state.current_qa_id
        
        # Message input - at root level to avoid Streamlit error
